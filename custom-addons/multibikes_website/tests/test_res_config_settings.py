...
class TestResConfigSettings(MultibikesWebsiteConfigTestCommon):
    ...
    def test_config_settings_remove_renting_period(self):
        ...
        config.renting_periods = [
            (3, self.past_period.id)
        ]  # Command 3: remove this ID, but do not delete it
        ...
        period_exists = (
            self.env["mb.renting.period"].browse(self.past_period.id).exists()
        )
        self.assertTrue(
            period_exists, "La période devrait toujours exister dans la base de données"
        )
