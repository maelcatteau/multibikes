...
class TestMBRentingPeriod(MultibikesWebsiteTestCommon):
    ...
    def test_find_period_for_date(self):
        ...
        period = self.env['mb.renting.period'].find_period_for_date(self.today + timedelta(days=5), self.main_company.id)
        self.assertEqual(period.id, self.current_period.id)
        
        # Date hors période
        period = self.env['mb.renting.period'].find_period_for_date(self.next_month + timedelta(days=10), self.main_company.id)
        ...
        self.assertNotEqual(period.id, self.current_period.id)  # Ne devrait pas trouver notre période de test qui a une compagnie spécifique
    ...
    def test_compute_products_to_configure(self):
        ...
        self.assertEqual(self.current_period.remaining_products_to_configure, 1)  # storable_product2 reste à configurer
