...
class TestProductProduct(MultibikesWebsiteProductTestCommon):
    ...
    def test_get_availabilities(self):
        ...
        start_date = (
            ...
            else datetime.strptime(self.start_date, "%Y-%m-%d %H:%M:%S")
        )
        end_date = (
            self.end_date
            if isinstance(self.end_date, datetime)
            else datetime.strptime(self.end_date, "%Y-%m-%d %H:%M:%S")
        )

        # Sans spécifier d'entrepôt (tous les entrepôts)
        availabilities = self.rental_product._get_availabilities(
        ...
        self.assertTrue(
            len(availabilities) > 0,
            "Des périodes de disponibilité devraient être générées",
        )
        ...
        self.assertEqual(
            first_period["quantity_available"],
            total_qty_main,
        ...
        outgoing_move_date = datetime.strptime(
            self.outgoing_move.date, "%Y-%m-%d %H:%M:%S"
        )
        ...
        self.assertEqual(
            availabilities_excluded[0]["quantity_available"], total_qty_excluded
        )
