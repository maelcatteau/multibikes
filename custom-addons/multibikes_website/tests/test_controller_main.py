...
class TestWebsiteSaleRentingCustomController(HttpCase, MultibikesWebsiteTestCommon):
    ...
    def test_renting_product_constraints(self):
        ...
        self.assertEqual(
            ...
            200,
            "La route devrait répondre avec un statut HTTP 200",
        )
        ...
        self.assertIn(
            "all_periods", result, "La réponse devrait contenir 'all_periods'"
        )
        ...
        self.assertIn(
            ...
            period_ids,
            "La période actuelle devrait être dans la réponse",
        )
        self.assertIn(
            future_period.id,
            period_ids,
            "La période future devrait être dans la réponse",
        )
        self.assertIn(
            closed_period.id,
            period_ids,
            "La période fermée devrait être dans la réponse",
        )
