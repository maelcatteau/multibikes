...
class TestResCompany(MultibikesWebsiteConfigTestCommon):
    ...
    def test_get_dynamic_renting_minimal_duration_future_period(self):
        ...
        reference_date = self.tomorrow + timedelta(
            days=1
        )  # Pour être sûr d'être dans la période future
    ...
    def test_get_dynamic_renting_minimal_duration_no_period(self):
        ...
        self.assertEqual(
            result["duration"], 1, "La durée minimale par défaut devrait être de 1"
        )
        self.assertEqual(result["unit"], "day", "L'unité par défaut devrait être 'day'")
        self.assertIsNone(
            result["start_date"], "Il ne devrait pas y avoir de date de début"
        )
        self.assertIsNone(
            result["end_date"], "Il ne devrait pas y avoir de date de fin"
        )
