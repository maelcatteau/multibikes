...
class RentalExtensionWizard(models.TransientModel):
    ...
    mb_order_id = fields.Many2one(
        "sale.order", string="Commande de location", required=True
    )
    ...
    mb_line_ids = fields.One2many(
        "rental.extension.wizard.line", "mb_wizard_id", string="Articles à prolonger"
    )
    ...
    def _onchange_dates(self):
        ...
        if (
            self.mb_end_date
            and self.mb_start_date
            and self.mb_end_date <= self.mb_start_date
        ):
            _logger.warning(
            ...
            raise UserError(
                _(
                    "La date de fin de prolongation doit être postérieure à la date de début."
                )
            )
        ...
        if self.mb_start_date and self.mb_start_date < now:
            ...
            raise UserError(
                _("La date de début de prolongation ne peut pas être dans le passé.")
            )
    ...
    def _prepare_extension_order_values(self):
        ...
        original_duration_days = (
            original_order.rental_return_date - original_order.rental_start_date
        ).days
    ...
    def _prepare_extension_line_values(self, extension_order, wizard_line):
        ...
        price_unit = self._calculate_extension_price(
            product, original_line, duration_days
        )
    ...
    def _calculate_extension_price(self, product, original_line, duration_days):
        ...
        try:
            ...
            is_bike_category = (
                category.id
                in self.env["product.category"].search([("name", "ilike", "vélo")]).ids
            )

            # Vérifier si la catégorie est une catégorie d'accessoires
            is_accessory_category = (
                category.id
                in self.env["product.category"]
                .search([("name", "ilike", "accessoire")])
                .ids
            )
            ...
            if is_bike_category:
                if total_duration_days <= 5:
                    _logger.debug(
                        "Prix de prolongation pour vélo: utilisation du prix original %s (durée totale <= 5 jours)",
                        default_price,
                    )
                    return default_price
                else:
                    _logger.debug(
                        "Prix de prolongation pour vélo: utilisation du prix spécial 4.5€ (durée totale > 5 jours)"
                    )
                    return 4.5  # 4.5€ par jour pour les jours de prolongation
            elif is_accessory_category:
                if total_duration_days <= 3:
                    _logger.debug(
                        "Prix de prolongation pour accessoire: utilisation du prix original %s (durée totale <= 3 jours)",
                        default_price,
                    )
                    return default_price
                else:
                    _logger.debug(
                        "Prix de prolongation pour accessoire: utilisation du prix spécial 2.5€ (durée totale > 3 jours)"
                    )
                    return 2.5  # 2.5€ par jour pour les jours de prolongation
            else:
                # Prix par défaut pour les autres catégories
                _logger.debug(
                    "Prix de prolongation pour catégorie %s: utilisation du prix par défaut %s",
                    category.name,
                    default_price,
                )
                return default_price
    ...
    def _handle_pickings(self, original_order, extension_order):
        ...
        for wizard_line in self.mb_line_ids.filtered(lambda l: l.mb_selected):
            ...
            if original_line:
                ...
                prolonged_qty = min(
                    wizard_line.mb_quantity, original_line.product_uom_qty
                )
                ...
                _logger.info(
                    ...
                    original_line.product_id.name,
                    original_line.id,
                    original_line.qty_returned,
                    new_returned_qty,
                )
        ...
        for extension_line in extension_order.order_line:
            ...
            _logger.info(
                ...
                extension_line.product_id.name,
                extension_line.id,
                extension_line.product_uom_qty,
            )
        ...
        if extension_order.state in ["draft", "sent"]:
            _logger.info(
                "Confirmation de la commande de prolongation %s", extension_order.name
            )
    ...
    def create_extension_order(self):
        ...
        for wizard_line in self.mb_line_ids.filtered(lambda l: l.mb_selected):
            ...
            if wizard_line.mb_quantity > available_qty:
                _logger.warning(
                    ...
                    wizard_line.mb_product_id.name,
                    wizard_line.mb_quantity,
                    available_qty,
                )
                raise UserError(
        ...
        for wizard_line in self.mb_line_ids.filtered(lambda l: l.mb_selected):
            ...
            line_vals = self._prepare_extension_line_values(
                self.env["sale.order"], wizard_line
            )
        ...
        if not extension_order_lines_vals:
            ...
            raise UserError(
                _(
                    "Aucun article n'a été sélectionné pour la prolongation. Veuillez sélectionner au moins un article."
                )
            )
        ...
        with self.env.cr.savepoint():
            ...
            extension_order = (
                self.env["sale.order"]
                .with_context(ignore_sequence=True)
                .create(extension_order_vals)
            )
