...
class RentalExtensionWizardLine(models.TransientModel):
    ...
    mb_wizard_id = fields.Many2one(
        "rental.extension.wizard", string="Assistant", required=True, ondelete="cascade"
    )
    mb_order_line_id = fields.Many2one(
        "sale.order.line", string="Ligne de commande originale", required=True
    )
    mb_product_id = fields.Many2one(
        "product.product", string="Produit à prolonger", required=True
    )
    mb_product_name = fields.Char(string="Description", required=True)
    mb_quantity = fields.Float(
        string="Quantité en prolongation", required=True, default=1.0
    )
    mb_uom_id = fields.Many2one(
        "uom.uom", string="Unité de mesure de prolongation", required=True
    )
    mb_selected = fields.Boolean(
        string="Sélectionné",
        default=True,
        help="Cochez cette case pour inclure ce produit dans la prolongation",
    )
    mb_available_qty = fields.Float(
    ...
    virtual_available_at_date = fields.Float(
        string="Quantité prévue", compute="_compute_qty_at_date"
    )
    qty_available_today = fields.Float(
        string="Quantité disponible aujourd'hui", compute="_compute_qty_at_date"
    )
    free_qty_today = fields.Float(
        string="Quantité libre aujourd'hui", compute="_compute_qty_at_date"
    )
    scheduled_date = fields.Datetime(
        string="Date planifiée", compute="_compute_qty_at_date"
    )
    forecast_expected_date = fields.Datetime(
        string="Date prévue", compute="_compute_qty_at_date"
    )
    warehouse_id = fields.Many2one(
        "stock.warehouse", string="Entrepôt", compute="_compute_qty_at_date"
    )
    move_ids = fields.One2many(
        "stock.move", string="Mouvements de stock", compute="_compute_qty_at_date"
    )
    qty_to_deliver = fields.Float(
        string="Quantité à livrer", compute="_compute_qty_at_date"
    )
    is_mto = fields.Boolean(
        string="Produit à la commande", compute="_compute_qty_at_date"
    )
    display_qty_widget = fields.Boolean(
        string="Afficher le widget de quantité", default=True
    )
    product_id = fields.Many2one(
        "product.product", string="Produit original", compute="_compute_product_data"
    )
    product_uom = fields.Many2one(
        "uom.uom", string="Unité de mesure originale", compute="_compute_product_data"
    )
    product_uom_qty = fields.Float(
        string="Quantité originale", compute="_compute_product_data"
    )
    ...
    def _compute_available_qty(self):
        ...
        for line in self:
            if line.mb_order_line_id:
                line.mb_available_qty = (
                    line.mb_order_line_id.qty_delivered
                    - line.mb_order_line_id.qty_returned
                )
                _logger.debug(
                    "Calcul quantité disponible: produit=%s, ligne=%s, livrée=%s, retournée=%s, disponible=%s",
                    line.mb_product_id.name,
                    line.mb_order_line_id.id,
                    line.mb_order_line_id.qty_delivered,
                    line.mb_order_line_id.qty_returned,
                    line.mb_available_qty,
                )
    ...
    def _compute_qty_at_date(self):
        ...
        for line in self:
            ...
            qty_available = line.product_id.with_context(
                warehouse=warehouse.id, to_date=scheduled_date
            ).virtual_available
            ...
            line.is_mto = (
                ...
                and float_compare(
                    line.product_id.nbr_reordering_rules,
                    0,
