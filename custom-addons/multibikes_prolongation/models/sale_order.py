...
class SaleOrder(models.Model):
    ...
    def _compute_mb_has_rentable_lines(self):
        ...
        _logger.debug(
            "Calcul des commandes avec articles prolongeables pour %d commandes",
            len(self),
        )
    ...
    def _compute_mb_extension_count(self):
        ...
        for order in self:
            ...
            if count > 0:
                _logger.debug(
                    "Commande %s: %d prolongation(s) trouvée(s)", order.name, count
                )
    ...
    def action_extend_rental(self):
        ...
        if not self.is_rental_order:
            _logger.warning(
                "Tentative de prolongation sur une commande non-location: %s", self.name
            )
            raise UserError(
                _(
                    "Cette commande n'est pas une location. Seules les commandes de location peuvent être prolongées."
                )
            )
        ...
        if self.state not in ["sale", "done"]:
            ...
            raise UserError(
                ...
                % (
                    dict(self._fields["state"]._description_selection(self.env)).get(
                        self.state
                    )
                )
            )
        ...
        _logger.info(
            "Ouverture de l'assistant de prolongation pour la commande %s", self.name
        )
        ...
        if not wizard_line_vals:
            _logger.warning(
                "Aucun article disponible pour prolongation dans la commande %s",
                self.name,
            )
            raise UserError(
                _(
        ...
        wizard = self.env["rental.extension.wizard"].create(
            {
                ...
                "mb_end_date": self.rental_return_date
                + timedelta(days=1),  # Par défaut, prolongation d'un jour
                "mb_line_ids": wizard_line_vals,
            }
        )

        _logger.info(
            "Assistant de prolongation créé pour la commande %s: wizard_id=%d",
            self.name,
            wizard.id,
        )
        ...
        return {
            ...
            "view_id": self.env.ref(
                "multibikes_prolongation.view_rental_extension_wizard_form"
            ).id,
            "target": "new",
            "context": context,
        }
