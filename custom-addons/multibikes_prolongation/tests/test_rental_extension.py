...
class TestRentalExtension(TransactionCase):
    ...
    def test_01_rental_extension_creation(self):
        ...
        for line in extension_order.order_line:
            self.assertEqual(line.qty_delivered, line.product_uom_qty, "Les articles devraient être marqués comme livrés")
    ...
    def test_02_partial_extension(self):
        ...
        self.assertEqual(extension_line.product_id.id, self.product_bike.id, "Le produit prolongé devrait être le vélo")
        self.assertEqual(extension_line.product_uom_qty, 1.0, "La quantité prolongée devrait être 1")
        
        # Vérifier que la quantité retournée dans la commande d'origine est correcte
        self.assertEqual(self.rental_line_bike.qty_returned, 1.0, "1 vélo devrait être marqué comme retourné")
        self.assertEqual(self.rental_line_accessory.qty_returned, 0.0, "L'accessoire ne devrait pas être marqué comme retourné")
